...
WITH '1.1.1.1' AS addr SELECT splitByChar('/', address) AS base_mask FROM  (SELECT arrayJoin(['1.1.1.0/24']) AS address) WHERE (IPv4CIDRToRange(toIPv4(addr), toUInt8(base_mask[2])).1) = toIPv4(base_mask[1])
